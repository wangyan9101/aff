...
/***/ function(module, exports, __webpack_require__) {
	  ...
	  _createClass(App, [{
	    ...
	    value: function patchNode(last_element, node, last_node) {
	      ...
	      // diff object
	          ...
	          if (node.style !== null) {
	            for (var _key9 in node.style) {
	              if (!last_node.style || node.style[_key9] != last_node.style[_key9]) {
	                last_element.style[_key9] = node.style[_key9];
	              }
	            }
	          }
	          if (last_node.style !== null) {
	            for (var _key10 in last_node.style) {
	              if (!node.style || !(_key10 in node.style)) {
	                last_element.style[_key10] = '';
	              }
	            }
	          }
	        }
	      ...
	      // class
	      for (var _key11 in node.classList) {
	        // should update
	        if (!last_node.classList || node.classList[_key11] != last_node.classList[_key11]) {
	          if (node.classList[_key11]) {
	            last_element.classList.add(_key11);
	          } else {
	            last_element.classList.remove(_key11);
	          }
	        }
	      }
	      for (var _key12 in last_node.classList) {
	        if (!node.classList || !(_key12 in node.classList)) {
	          last_element.classList.remove(_key12);
	        }
	      }
	
	      // attributes
	      for (var _key13 in node.attributes) {
	        if (!last_node.attributes || node.attributes[_key13] != last_node.attributes[_key13]) {
	          var value = node.attributes[_key13];
	          var valueType = typeof value === 'undefined' ? 'undefined' : _typeof(value);
	          if (valueType == 'string' || valueType == 'number') {
	            last_element.setAttribute(_key13, value);
	            last_element[_key13] = value;
	          } else if (valueType == 'boolean') {
	            if (value) {
	              last_element.setAttribute(_key13, true);
	              last_element[_key13] = true;
	            } else {
	              last_element.removeAttribute(_key13);
	              last_element[_key13] = false;
	            }
	          }
	        }
	      }
	      for (var _key14 in last_node.attributes) {
	        if (!node.attributes || !(_key14 in node.attributes)) {
	          last_element.removeAttribute(_key14);
	          last_element[_key14] = undefined;
	        }
	      }
	      ...
	      for (var _key15 in node.events) {
	        var k = elementSetEvent(last_element, _key15, node.events[_key15].bind(node));
	        event_keys[k] = true;
	      }
	...
	function e(tag) {
	  ...
	  for (var _len6 = arguments.length, args = Array(_len6 > 1 ? _len6 - 1 : 0), _key17 = 1; _key17 < _len6; _key17++) {
	  ...
	  _createClass(Node, [{
	    ...
	    value: function toElement(name, app) {
	      ...
	      if (this.attributes !== null) {
	        for (var _key18 in this.attributes) {
	          var value = this.attributes[_key18];
	          var valueType = typeof value === 'undefined' ? 'undefined' : _typeof(value);
	          if (valueType == 'string' || valueType == 'number') {
	            element.setAttribute(_key18, value);
	            element[_key18] = value;
	          } else if (valueType == 'boolean') {
	            if (value) {
	              element.setAttribute(_key18, true);
	              element[_key18] = true;
	            } else {
	              element.removeAttribute(_key18);
	              element[_key18] = false;
	            }
	          }
	        }
	      }
	      if (this.events !== null) {
	        for (var _key19 in this.events) {
	          // set event callback, bind current Node to callback
	          // constructor must not be arrow function to get proper 'this'
	          elementSetEvent(element, _key19, this.events[_key19].bind(this));
	        }
	      }
