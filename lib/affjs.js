...
/***/ function(module, exports, __webpack_require__) {
	...
	var skip = exports.skip = { skip_following_arguments: true };
	  ...
	  for (var i = 0; i < args.length; i++) {
	    ...
	    if (arg instanceof Node || arg instanceof Thunk) {
	      ...
	      for (var idx = 0; idx < arg.events.length; idx++) {
	        ...
	        node.setProperties(_defineProperty({}, 'on' + ev.ev_type, ev.fn));
	      }
	    } else if (arg === skip) {
	      break;
	  ...
	  _createClass(Node, [{
	    ...
	    value: function toElement(name, app) {
	      ...
	      if (this.attributes !== null) {
	        for (var _key20 in this.attributes) {
	          var value = this.attributes[_key20];
	          var valueType = typeof value === 'undefined' ? 'undefined' : _typeof(value);
	          if (valueType == 'string' || valueType == 'number') {
	            element.setAttribute(_key20, value);
	            element[_key20] = value;
	          } else if (valueType == 'boolean') {
	            if (value) {
	              element.setAttribute(_key20, true);
	              element[_key20] = true;
	            } else {
	              element.removeAttribute(_key20);
	              element[_key20] = false;
	            }
	          }
	        }
	      }
	      if (this.events !== null) {
	        for (var _key21 in this.events) {
	          // set event callback, bind current Node to callback
	          // constructor must not be arrow function to get proper 'this'
	          elementSetEvent(element, _key21, this.events[_key21].bind(this));
	        }
	      }
...
/***/ function(module, exports) {
	...
	var Event = function Event(ev_type, fn) {
	...
	var MutableState = exports.MutableState = function (_State) {
	  ...
	  _createClass(MutableState, [{
	    ...
	    value: function argsChanged(arg, last_arg) {
	      ...
	      if (arg === last_arg && (typeof arg === 'undefined' ? 'undefined' : _typeof(arg)) === 'object' && arg != null && arg.__aff_use_keys && arg.__aff_tick_update_at != this.patch_tick) {
	    ...
	    key: 'setupState',
	    value: function setupState(state, base_path, forceSetup) {
	      var _this2 = this;
